...
public class BSTImpl<T extends Comparable<T>> implements BST<T> {
	...
	private int heightRec(BSTNode<T> node, int cont) {
		if (!node.isEmpty()){
			int left = heightRec((BSTNode<T>) node.getLeft(), cont + 1);
			int right = heightRec((BSTNode<T>) node.getRight(), cont + 1);
	...
	private boolean removeAux(BSTNode<T> node) {
	...
	private void preOrderRec(BSTNode<T> node, List<T> aux) {
		if(!node.isEmpty()) {
			aux.add(node.getData());
			preOrderRec((BSTNode<T>) node.getLeft(), aux);
			preOrderRec((BSTNode<T>) node.getRight(), aux);
		}
	}
	...
	private void OrderRec(BSTNode<T> node, List<T> aux) {
		if(!node.isEmpty()){
			OrderRec((BSTNode<T>) node.getLeft(), aux);
			aux.add(node.getData());
			OrderRec((BSTNode<T>) node.getRight(), aux);
		}
	}
	...
	private void postOrderRec(BSTNode<T> node, List<T> aux) {
		if (!node.isEmpty()) {
			postOrderRec((BSTNode<T>) node.getLeft(), aux);
			postOrderRec((BSTNode<T>) node.getRight(), aux);
			aux.add(node.getData());
		}
	}
