...
public class AVLTreeImpl<T extends Comparable<T>> extends BSTImpl<T> implements AVLTree<T> {
	...
	// AUXILIARY
	protected int calcularLaBalance(BSTNode<T> node) {
		int result = 0;
		if (!node.isEmpty())
	...
	protected void rebalance(BSTNode<T> node) {
		...
		int balance =  calcularLaBalance(node);
		if (balance < -1) {
			if ( calcularLaBalance((BSTNode<T>) node.getRight()) > 0) {
				rightRotation((BSTNode<T>) node.getRight());
			}
			leftRotation(node);
		} else if (balance > 1) {
			if ( calcularLaBalance((BSTNode<T>) node.getLeft()) < 0) {
