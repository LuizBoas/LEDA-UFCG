...
public class RBTreeImpl<T extends Comparable<T>> extends BSTImpl<T> implements RBTree<T> {
   ...
   private int blackHeightLeft(RBNode<T> node) {
      ...
      if (node.isEmpty()) {
         ...
         height = blackHeightLeft((RBNode<T>) node.getLeft());
      } else {
         height = 1 + blackHeightLeft((RBNode<T>) node.getLeft());
      }
      return height;
   }
   ...
   private int blackHeightRight(RBNode<T> node) {
      ...
      if (node.isEmpty()) {
         ...
         height = blackHeightRight((RBNode<T>) node.getRight());
      } else {
         height = 1 + blackHeightRight((RBNode<T>) node.getRight());
      }
      return height;
   }
